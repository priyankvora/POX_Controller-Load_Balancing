...
class iplb (object):
  ...
  print "Let my load balancer work..."
...
"I have changed this part of code, which is Dynamic Weighted algorithm. It will pick server based on weighted given as starting of load balancing."
  def _pick_server (self, key, inport):
    ...
    """
    global select_server
    global counter
    global b
    global i
    server_list = self.live_servers.keys()

    if (counter < int(weightage[i]) and select_server < len(self.live_servers)):
      counter = counter + 1
    elif (counter >= int(weightage[i]) and select_server + 1 < len(self.live_servers)):
      select_server = select_server + 1
      counter = 1
      i = i + 1
    else:
      i = 0
      counter = 1
      select_server = 0
    return server_list[select_server]
...
def launch (ip, servers, weightage):
